...
Inspired by [Actual Budget](https://github.com/actualbudget/actual) and [Firefly III](https://github.com/firefly-iii/firefly-iii).
...
- **Transaction tracking** — Manual entry, CSV import, soft-delete with undo
- **Auto-categorisation** — Payee rule engine with exact, contains, and regex matching
- **Monthly budgets** — Progress bars, budget vs. actual, copy-last-month
- **Reporting** — Monthly comparison, category breakdown, net worth, 12-month cash flow
- **Recurring transactions** — Scheduled jobs via BullMQ; skip and mark-paid actions
- **MFA** — TOTP (authenticator app) and Passkeys (WebAuthn / biometrics)
- **Settings** — Profile, password change, session management, GDPR data export, account deletion
- **Dark mode** — Toggleable light, dark, or system theme
- **Privacy by design** — No outbound HTTP calls, no analytics, no telemetry
- **OpenAPI docs** — Browseable Swagger UI at `/api/docs`
...
| Layer | Technology |
|---|---|
| Backend | Fastify 5, TypeScript 5, Node.js 22 |A
| ORM | Prisma 6, PostgreSQL 16 |
| Auth | Custom session auth (TOTP + WebAuthn via @simplewebauthn) |
...
```bash
cp .env.example .env
# Fill in all values — especially generate strong secrets:
#   openssl rand -hex 32
...
| Command | Description |
|---|---|
| `docker compose -f docker-compose.dev-full.yml up --build` | **Full Docker dev** — first run |
| `docker compose -f docker-compose.dev-full.yml up` | Full Docker dev — subsequent starts |
| `pnpm dev` | Start all apps locally in watch mode (Option B) |
| `pnpm build` | Build all packages and apps |
| `pnpm test` | Run all unit tests |
| `pnpm typecheck` | TypeScript type-check all packages |
| `pnpm lint` | ESLint across all packages |
| `pnpm --filter @finance/db db:migrate:dev` | Run Prisma migrations (dev) |
...
| 2 | Accounts & transactions | ✅ Done |
| 3 | Categories & budgets | ✅ Done |
| 4 | Auto-categorisation | ✅ Done |
| 5 | Reporting | ✅ Done |
| 6 | Recurring transactions | ✅ Done |
| 7 | Settings, security & polish | ✅ Done |
| 8 | Documentation & hardening | ✅ Done |
...
Key controls:
- All routes authenticated; user-scoped DB queries
- TOTP secrets AES-256-GCM encrypted at rest
- Passwords bcrypt (cost 12)
- Rate limiting on auth routes (5 req/min per IP)
- HttpOnly, Secure, SameSite=Strict session cookies
- Content-Security-Policy via Caddy

---

## Privacy

- Zero telemetry, zero outbound HTTP calls (unless SMTP is configured)
- No third-party scripts or fonts
- GDPR: data export (`GET /api/export`) and deletion (`DELETE /api/account`)
- Backend network is `internal: true` in Docker Compose
